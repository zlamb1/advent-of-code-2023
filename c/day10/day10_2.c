...
void replace_start(pipe* start, size_t max_line_len) {
    pipe* prev = start->prev, *next = start->next;
    size_t index = start->index;
    if (index + max_line_len == prev->index && index - max_line_len == next->index)
        start->c = '|';
    if (index + 1 == prev->index && index - 1 == next->index)
        start->c = '-';
    if (index + 1 == prev->index && index - max_line_len == next->index)
        start->c = 'L';
    if (index - 1 == prev->index && index - max_line_len == next->index)
        start->c = 'J';
    if (index + max_line_len == prev->index && index + 1 == next->index)
        start->c = 'F';
    if (index + max_line_len == prev->index && index - 1 == next->index)
        start->c = '7';
}
...
size_t parse_pipes(str_buf_t* buf) {
    ...
    replace_start(start, buf->max_line_len);
    size_t counter = 0;
    char corner = '\0'; 
    bool inside = false;
    for (size_t i = 0; i < buf->len; i++) {
